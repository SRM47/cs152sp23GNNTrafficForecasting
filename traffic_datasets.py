...
class PemsBayDatasetLoaderSingleValue(object):
    ...
    def __init__(self, raw_data_dir=os.path.join(os.getcwd(), "data")):
        super(PemsBayDatasetLoaderSingleValue, self).__init__()
    ...
    def _read_web_data(self):
        ...
        self.X_regular = torch.from_numpy(X)
    ...
    def _generate_task(self, num_timesteps_in: int = 12, num_timesteps_out: int = 12):
        ...
        features, target = [], []
        features_regular, target_regular = [], []
        for i, j in indices:
            features.append((self.X[:, :, i : i + num_timesteps_in]).numpy())
            target.append((self.X[:, 0, i + num_timesteps_in : j]).numpy()) #only change here
            features_regular.append((self.X_regular[:, :, i : i + num_timesteps_in]).numpy())
            target_regular.append((self.X_regular[:, 0, i + num_timesteps_in : j]).numpy()) #only change here

        self.features = features
        self.targets = target
        self.features_regular = features_regular
        self.targets_regular = target_regular
    ...
    def get_dataset(
        ...
        dataset_normalized = StaticGraphTemporalSignal(
            self.edges, self.edge_weights, self.features, self.targets
        )
        dataset_regular = StaticGraphTemporalSignal(
            self.edges, self.edge_weights, self.features_regular, self.targets_regular
        )
        return dataset_normalized, dataset_regular, self.means, self.stds
